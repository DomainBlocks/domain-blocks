...
public class MongoEventStoreDependencyTests
{
    ...
    public void ConstructionSurface_ParameterTypes_AreCodecContractsOrStoreTypes()
    {
        ...
        var parameterTypes = typeof(MongoEventStore<>).GetConstructors(all)
            .SelectMany(c => c.GetParameters())
            .Concat(typeof(MongoEventStore).GetMethods(all).Where(m => m.Name == "Create").SelectMany(m => m.GetParameters()))
