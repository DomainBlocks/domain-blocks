...
/// this test does.
/// </summary>
public class PostgresEventStoreDependencyTests
{
    ...
    public void ConstructionSurface_ParameterTypes_AreCodecContractsOrStoreTypes()
    {
        ...
        var parameterTypes = typeof(PostgresEventStore<>).GetConstructors(all)
            .SelectMany(c => c.GetParameters())
            .Concat(typeof(PostgresEventStore).GetMethods(all).Where(m => m.Name == "Create").SelectMany(m => m.GetParameters()))
