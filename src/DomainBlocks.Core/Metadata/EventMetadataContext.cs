...
{
    private readonly Dictionary<string, string> _metaDataEntries = new();
    private readonly Dictionary<string, Func<string>> _lazyEntries = new();
    ...
    {
        var entries = _metaDataEntries.Concat(EvaluateLazyEntries().Concat(additionalEntries));
        return EventMetadata.FromKeyValuePairs(entries);
    }
