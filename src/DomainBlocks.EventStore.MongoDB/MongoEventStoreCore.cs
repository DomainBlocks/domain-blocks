...
/// <summary>
/// The store proper: sequenced appender, event log collection and change-stream subject over a client the caller
/// owns. Wired by <see cref="MongoEventStore.Create{TEvent}"/> and <see cref="MongoEventStoreBuilder{TEvent}"/>, which
/// present it behind <see cref="MongoEventStore{TEvent}"/>.
/// </summary>
internal static class MongoEventStoreCore
{
    ...
    public static MongoEventStoreCore<TEvent> Create<TEvent>(
        ...
        where TEvent : notnull
    {
        ...
        return new MongoEventStoreCore<TEvent>(sequencedAppender, eventLog, eventCodec, logger);
    }
}

internal sealed class MongoEventStoreCore<TEvent> : IEventStore<TEvent, string, StreamPosition, LogPosition>
    where TEvent : notnull
{
