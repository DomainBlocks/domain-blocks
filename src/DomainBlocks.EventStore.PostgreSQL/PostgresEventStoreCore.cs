...
/// <summary>
/// The store proper: appender, reader and replication feed over a data source the caller owns. Wired by
/// <see cref="PostgresEventStore.Create{TEvent}"/> and <see cref="PostgresEventStoreBuilder{TEvent}"/>, which present
/// it behind <see cref="PostgresEventStore{TEvent}"/>.
/// </summary>
internal static class PostgresEventStoreCore
{
    /// <remarks>
    ...
    /// </remarks>
    public static PostgresEventStoreCore<TEvent> Create<TEvent>(
        NpgsqlDataSource dataSource,
        IEventCodec<TEvent, PostgresEventData, string> eventCodec,
        PostgresEventStoreOptions options,
        ILogger? logger,
        string? replicationConnectionStringFallback = null)
        where TEvent : notnull
    {
        ...
        return new PostgresEventStoreCore<TEvent>(appender, reader, feed, eventCodec, logger);
    }
...
internal sealed class PostgresEventStoreCore<TEvent> : IEventStore<TEvent, string, StreamPosition, LogPosition>
    where TEvent : notnull
{
    ...
    internal PostgresEventStoreCore(
        IAppender appender,
